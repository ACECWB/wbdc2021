...
def adjust(df, key, feature):
    if key == 'uid':
        ...
        df.loc[(df['pt_d'] >= 8) & (df['coldu'] == 1), feature] = (
                    (df[(df['pt_d'] >= 8) & (df['coldu'] == 1)][feature] - mean8) / std8 * std7 + mean7)
    return df
...
def build_graph(df, f1, f2):
    G = nx.Graph()
    df_weight = df.groupby([f1, f2], as_index=False)['gender'].agg({'weight': 'count', }).reset_index().drop('index',
                                                                                                             axis=1)
...
def make_feature(df):
    ...
    print('广告')
    key = 'adv_id'
    feature_target = ['uid', 'age', 'city', 'device_name', 'device_size', 'career', 'residence', 'gender',
                      'adv_prim_id', 'slot_id', 'spread_app_id']
    for target in tqdm(feature_target):
        if key + '_' + target + '_nunique' not in nunique_group:
            nunique_group.append(key + '_' + target + '_nunique')
            tmp = group_fea(df, key, target)
...
if __name__ == "__main__":
    ...
    Parallel(n_jobs=fold)(
        delayed(atom_makefea)(i, trn_idx, val_idx, df0, df1, test_df_raw, fold) for i, (trn_idx, val_idx) in
        enumerate(skf.split(df0, df0['pt_d'])))
